...
const steps = [
  ...
  { title: "Verify your identity", body: "Upload the documents needed to protect buyers and sellers on Afro Mart.", image: kycScreen.url },
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] bg-background">
      <section className="brand-soft px-4 py-14 text-center sm:px-6">
        <span className="text-sm font-bold uppercase text-brand-green">Sell on Afro Mart</span>
        ...
        <Link to="/auth" search={{ redirect: "/account" }} className="mt-8 inline-block"><Button size="lg"><UserRound className="h-4 w-4" /> Create seller account <ArrowRight className="h-4 w-4" /></Button></Link>
      </section>
      <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <div className="grid gap-x-8 gap-y-16 sm:grid-cols-2 lg:grid-cols-4">
          {steps.map((step, index) => (
            <article key={step.title}>
              <AppScreen src={step.image} alt={`Afro Mart seller step ${index + 1}: ${step.title}`} className="w-full" />
