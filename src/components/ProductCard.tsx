    ...
    <Card className="group flex flex-col overflow-hidden border bg-card transition-shadow hover:shadow-md">
      ...
      <CardContent className="flex flex-1 flex-col p-4">
        ...
        <Link to="/products/$slug" params={{ slug: product.slug }}>
          <h3 className="mt-1 font-heading text-base font-semibold leading-tight text-foreground hover:text-primary">
            {product.name}
          </h3>
        </Link>
        <p className="mt-2 line-clamp-2 text-sm text-muted-foreground">{product.description ?? ""}</p>
        <p className="mt-auto pt-4 font-semibold text-foreground">{formatPrice(product.price)}</p>
      </CardContent>
