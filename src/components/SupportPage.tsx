...
const topics = [
  {
    ...
    body: "Track a parcel, change an address or report a late order.",
  },
  {
    icon: CreditCard,
    value: "payments",
    title: "Payments & refunds",
    body: "Card, bank transfer and mobile money questions, plus refunds.",
  },
  {
    icon: Store,
    value: "selling",
    title: "Selling on Afromart",
...
const faqs = [
  {
    ...
    a: "Open Orders in your account and choose the order. You will see each step from confirmation to delivery.",
  },
  {
    q: "How do I reach a seller?",
    a: "Open the product and tap “Chat with seller”. Your conversations live under Messages.",
  },
  {
    q: "Which payment methods are supported?",
    a: "Cards, bank transfer and mobile money as they go live in each country.",
  },
  {
    q: "How do I become a seller?",
    a: "Open Start selling and follow registration, business details, verification and approval.",
  },
  {
    q: "Can I change the app language or appearance?",
    a: "Yes. Settings lets you switch language and choose light or dark.",
  },
];
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-7 sm:px-6 sm:py-10 lg:px-10">
      <section className="grid items-center gap-6 rounded-2xl border bg-card p-5 sm:p-7 lg:grid-cols-2">
        ...
        <img
          src={supportHero}
          alt="An Afromart support agent wearing a headset"
      ...
      <section className="mt-8 border-y py-6">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Help with your orders, deliveries, account and selling.
        </p>
        <Button asChild className="mt-4">
          <Link to="/support-chat">Chat with Afromart</Link>
        </Button>
      </section>
          ...
          {!user ? (
            <div className="mt-4 rounded-xl border bg-card p-6">
              <p className="text-sm text-muted-foreground">
                Sign in to open a request and see replies from our team.
              </p>
              <Button asChild className="mt-4">
                <Link to="/auth" search={{ redirect: "/support" }}>
                  Sign in
                </Link>
              </Button>
            </div>
            ...
            <>
              <form onSubmit={submit} className="mt-4 space-y-3 rounded-xl border bg-card p-5">
                <div>
                  <label htmlFor="support-subject" className="text-sm font-semibold">
                    Subject
                  </label>
                  <Input
                    id="support-subject"
                    ...
                    placeholder="Late delivery for order 4F2C…"
                    className="mt-1.5"
                  />
                </div>
                <div>
                  <label htmlFor="support-message" className="text-sm font-semibold">
                    What happened?
                  </label>
                  <Textarea
                    id="support-message"
              ...
              <ul className="mt-5 space-y-3">
                {(tickets ?? []).map((ticket) => {
                  const messages = [...(ticket.support_messages ?? [])].sort((a, b) =>
                    ...
                    <li key={ticket.id} className="rounded-xl border bg-card p-4">
                      ...
                      >
                        <span className="flex items-center justify-between gap-3">
                          <span className="min-w-0 truncate font-heading font-bold">
                            {ticket.subject}
                          </span>
                          <span className="shrink-0 rounded-full bg-secondary px-2 py-0.5 text-[11px] font-bold uppercase text-primary">
                            {ticket.status}
                          </span>
                        </span>
                      ...
                      {open ? (
                        <div className="mt-3 space-y-2 border-t pt-3">
                          {messages.map((entry) => (
                            <div
                              key={entry.id}
                              className={`rounded-lg px-3 py-2 text-sm ${entry.sender === "user" ? "bg-primary/10" : "bg-secondary"}`}
                            >
                              <p className="whitespace-pre-wrap">{entry.body}</p>
                              <p className="mt-1 text-[10px] text-muted-foreground">
                                {entry.sender === "user" ? "You" : "Afromart support"} ·{" "}
