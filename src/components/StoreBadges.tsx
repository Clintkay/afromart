...
export function StoreBadges({ className = "" }: { className?: string }) {
  return (
    <div className={`flex flex-wrap items-center gap-3 ${className}`}>
      <Link
        to="/coming-soon"
        className="flex items-center gap-3 rounded-2xl border border-brand-gold/40 bg-brand-ink px-5 py-3 text-white shadow-lg transition-transform hover:-translate-y-0.5"
      >
        <Apple className="h-6 w-6 text-brand-gold" />
      ...
      <Link
        ...
        className="flex items-center gap-3 rounded-2xl border border-brand-gold/40 bg-brand-ink px-5 py-3 text-white shadow-lg transition-transform hover:-translate-y-0.5"
      >
        <Play className="h-6 w-6 text-brand-leaf" />
        <span className="flex flex-col leading-tight">
