...
export async function supportChatResponse(request: Request) {
  ...
  if (!url || !publicKey || !key)
    return new Response("Support is not configured. Please contact human support.", {
      status: 503,
    });
  ...
  if (!parsed.success)
    return new Response("Invalid conversation. Start a new chat.", { status: 400 });
  ...
  if (
    history.some((m) => m.content.length > 10000) ||
    history.reduce((n, m) => n + m.content.length, 0) > 100000
  )
    return new Response("This conversation is too long. Please start a new chat.", { status: 400 });
  const title = history.find((m) => m.role === "user")?.content.slice(0, 80) || "Support chat";
  const { error: saveError } = await db
    .from("ai_support_threads")
    .update({ messages: messages as Json, title, updated_at: new Date().toISOString() })
    .eq("id", id);
  if (saveError)
    return new Response("Your message could not be saved. Please try again.", { status: 503 });
    ...
    fetch: async (input, init) => {
      ...
      if (!response.ok) {
        ...
        const body = await response
          ...
          .catch(() => ({}));
        upstreamMessage =
          body.message ?? body.error?.message ?? `Support is unavailable (${response.status}).`;
        if (response.status === 402 || response.status === 403) {
          const { error: e } = await supabaseAdmin
            .from("ai_support_access")
            .upsert({ id: "support", reason: upstreamMessage, status: response.status });
          if (e) console.error("Could not save assistant availability");
        }
      }
      return response;
    },
  ...
  const response = result.toUIMessageStreamResponse({
    ...
    onError: () =>
      upstreamMessage ||
      "The assistant could not answer. Please use human support or try again later.",
    onFinish: async ({ messages: completed }) => {
      const { error: e } = await db
        .from("ai_support_threads")
        .update({ messages: completed as unknown as Json, updated_at: new Date().toISOString() })
        .eq("id", id)
        .eq("user_id", auth.user.id);
      if (e) throw new Error("Could not save your conversation.");
    },
  });
  ...
  if (upstreamStatus !== 200) {
    await reader.cancel();
    return new Response(upstreamMessage, { status: upstreamStatus });
  }
  ...
  return new Response(
    new ReadableStream({
      async start(c) {
        try {
          for (const part of [first, next]) if (!part.done) c.enqueue(part.value);
          while (!next.done) {
