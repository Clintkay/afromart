...
- [x] Removed delivery payment. Card remains enabled; transfer disabled pending merchant bank/provider configuration.
...
- [ ] Connect landing app CTAs directly to the buying marketplace.
- [ ] Fix checkout address selection, trusted totals, payment retry, and payment confirmation checks.
- [ ] Add web seller verification entry with private credential upload and approval status.
- [ ] Add ownership policies for credential storage and verify access.
- [ ] Verify browsing, cart, chat entry, and seller verification on phone and desktop.
